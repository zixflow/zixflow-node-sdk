...
describe('CollectionRecords - createCollectionRecord', () => {
  let collectionRecords: CollectionRecords;
  const apiKey = 'your-api-key';
  const domain = 'https://api.zixflow.com';
  const collectionId = 'people';
  const api_url_save = `${domain}/api/v1/collection-records/${collectionId}`;
  ...
  it("should save a new record to the specified collection", async () => {
    ...
    const result = await collectionRecords.createCollectionRecord(collectionId, newRecord);

    expect(mockedValidateParameters).toHaveBeenCalledWith({ collectionId, recordData: newRecord }, ['collectionId', 'recordData']);
