...
describe('ListEntries - getListOfListEntries', () => {
  let listEntries: ListEntries;
  const apiKey = 'your-api-key';
  const domain = 'https://api.zixflow.com';
  const listId = 'people';
  const api_url = `${domain}/api/v1/list-entries/${listId}/query`;
  ...
  it('should return list entries', async () => {
    ...
    const result = await listEntries.getListOfListEntries(listId , filterData);

    expect(mockedValidateParameters).toHaveBeenCalledWith({ listId }, ['listId']);
